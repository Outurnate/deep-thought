...
BOOST_AUTO_TEST_CASE(TrivialTransform)
{
  ...
  BOOST_WARN_THROW(piece(1, 1), std::out_of_range);
  field.ApplyTransform(piece.GetTransform(0, 0, FieldElement::RED));
  ...
  BOOST_REQUIRE_EQUAL(field(3, 0), FieldElement::RED);
}
...
BOOST_AUTO_TEST_CASE(FieldExtents)
{
  ...
  field.ApplyTransform(piece.GetTransform(0,                                   0,                                     FieldElement::RED));
  field.ApplyTransform(piece.GetTransform(field.GetWidth() - piece.GetWidth(), 0,                                     FieldElement::RED));
  field.ApplyTransform(piece.GetTransform(0,                                   field.GetHeight() - piece.GetHeight(), FieldElement::RED));
  field.ApplyTransform(piece.GetTransform(field.GetWidth() - piece.GetWidth(), field.GetHeight() - piece.GetHeight(), FieldElement::RED));
  BOOST_REQUIRE_EQUAL(field(0,                    0),                     FieldElement::RED);
